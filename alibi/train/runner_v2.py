...
def summarise(state, elapsed: float) -> None:
    ...
    lines = [
        ...
        "",
        "| Arm | Steps | Visible pass first10 to last10 | Cheat rate first10 to last10 | Mean think chars | A1/A2 flag rate |",
        "|---|---|---|---|---|---|",
    ]
    for entry in state.entries:
        ...
        lines.append(
            f"| {entry['arm']} | {len(summaries)} | {band(vis)} | {band(cheat)} "
            f"| {statistics.fmean(think):.0f} | {band(flag)} |"
        )
    lines += [
        "",
